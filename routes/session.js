...
function SessionHandler (db) {
    ...
    function validateSignup(username, password, verify, email, errors) {
        ...
        if (!USER_RE.test(username)) {
            ...
            console.log(errors['username_error']);
            return false;
        }
        if (!PASS_RE.test(password)) {
            errors['password_error'] = "invalid password.";
            console.log(errors['password_error']);
            return false;
        }
        if (password != verify) {
            errors['verify_error'] = "password must match";
            console.log(errors['verify_error']);
            return false;
        }
        if (email != "") {
    ...
    this.handleSignup = function(req, res, next) {
        ...
        var errors = {'username': username, 'password': password, 'email': email}

        console.log('User validation: '+username+' , '+ password+' , '+ verify+' , '+ email);
        ...
        if (validateSignup(username, password, verify, email, errors)) {
            ...
            return res.render("signup", { username: errors['username'], username_error: errors['username_error'], password: errors['password'], password_error: errors['password_error'], verify_error: errors['verify_error'], email: errors['email'], email_error: errors['email_error'] });
